...
*`IT_Admins` membership — including `bob.jones`, the service account targeted later in the lab, and `sarah.connor`, the account used to launch the Kerberoasting attack.*
...
From Kali, I used Impacket's `GetUserSPNs`, authenticating as the compromised `sarah.connor` account, to request the service ticket for `bob.jones`'s SPN (`MSSQLSvc/dc01.corp.local:1433`). The domain controller returned an encrypted TGS ticket using **RC4 encryption (type 0x17)** — a weak, legacy cipher that attack tools deliberately request because RC4 hashes are far faster to crack offline than modern AES.

<img src="Active%20Directory%20Images/Pn145.png" width="700">

*Impacket `GetUserSPNs` requesting the Kerberoastable service ticket for `bob.jones`. The authentication credential is redacted.*

On the defender's side, this request generated **Windows Security Event ID 4769**. I wrote a PowerShell script to pull the event and export the key fields — the requesting account, the target service, the encryption type, and the source IP — as structured JSON. The captured event shows `sarah.connor@CORP.LOCAL` requesting the service ticket for `bob.jones`, with encryption type `0x17` from the Kali host — the exact signature used to identify the Kerberoasting attack in this lab.

<img src="Active%20Directory%20Images/Pn209.png" width="700">

*Event 4769 captured on DC01: `sarah.connor` requesting an RC4 (`0x17`) ticket for `bob.jones` from the Kali host (`192.168.64.7`).*
...
<img src="Active%20Directory%20Images/Pn154.png" width="700">

*Disabling pre-authentication on `rachel.green` with `Set-ADAccountControl` and verifying that `DoesNotRequirePreAuth` is set to `True`.*

<img src="Active%20Directory%20Images/Pn166.png" width="700">
...
<img src="Active%20Directory%20Images/Pn180.png" width="700">

*Event 4768 captured on DC01: RC4 (`0x17`) authentication request for `rachel.green` from the Kali host (`192.168.64.7`).*

## Attack 3 — Pass-the-Hash (T1550.002)

Pass-the-Hash is different from the two roasting attacks: it skips cracking entirely. Windows NTLM authentication doesn't use the plaintext password to log in — it uses the password's NTLM hash. So if an attacker obtains the hash, they can authenticate as that user directly, without ever knowing the password. This is what makes it dangerous: the stolen hash remains usable until the account password is changed and a new hash is generated.

It's a two-part attack — first obtain a hash, then use it. To get the hashes, I ran Impacket's `secretsdump` from Kali against DC01, which dumped the NTLM hashes for the domain accounts. The raw `secretsdump` output is intentionally not shown in the public README because it contains reusable NTLM hashes, machine secrets, cached credentials, LSA secrets, and DPAPI material.

I then used the Administrator hash with Impacket's `psexec` to authenticate to DC01 as **Administrator using only the hash — no password**. The successful connection to the writable `ADMIN$` share and remote service creation confirm that the authentication worked.

<img src="Active%20Directory%20Images/Pn189.png" width="700">

*Impacket `psexec` authenticating to DC01 as Administrator using only the NTLM hash. The hash is redacted.*
...
<img src="Active%20Directory%20Images/Pn192.png" width="700">
